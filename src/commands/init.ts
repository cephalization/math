...
import { PROMPT_TEMPLATE, TASKS_TEMPLATE, LEARNINGS_TEMPLATE } from "../templates";
...
export async function init(options: { skipPlan?: boolean } = {}) {
  const todoDir = join(process.cwd(), "todo");

  if (existsSync(todoDir)) {
    console.log(`${colors.yellow}todo/ directory already exists${colors.reset}`);
    return;
  }
  ...
  console.log(`  ${colors.cyan}PROMPT.md${colors.reset}    - System prompt with guardrails`);
  ...
  console.log(`  1. Edit ${colors.cyan}todo/TASKS.md${colors.reset} to add your tasks`);
  console.log(`  2. Customize ${colors.cyan}todo/PROMPT.md${colors.reset} for your project`);
  console.log(`  3. Run ${colors.cyan}math run${colors.reset} to start the agent loop`);
}
